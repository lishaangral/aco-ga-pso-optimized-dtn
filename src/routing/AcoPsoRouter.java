...
/**
 * Ablation Study: ACO + PSO router for THE ONE (No GA).
 *
 * This "Local Trap" model lacks evolutionary path mutation.
 * ACO: historical memory via pheromone + encounter history.
 * PSO: adaptive replication pressure and kinematic velocity evaluation.
 * GA mutation is bypassed, risking stagnation in local optima.
 */
public class AcoPsoRouter extends ActiveRouter {
    ...
    public static final String SWARM_SIZE_S = "swarmSize";
    ...
    private static final String MSG_COPIES_PROP = "AcoPsoRouter.copies";
    private static final String MSG_CLASS_MASK_PROP = "AcoPsoRouter.classMask";
    ...
    public AcoPsoRouter(Settings s) {
        super(s);
        ...
        this.swarmSize = getIntOrDefault(s, SWARM_SIZE_S, 8); // Limits candidates evaluated by PSO
    ...
    protected AcoPsoRouter(AcoPsoRouter r) {
        super(r);
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            if (other.getRouter() instanceof AcoPsoRouter) {
                syncDelivered((AcoPsoRouter) other.getRouter());
            }
        } else {
    ...
    private HybridChoice findBestHybridChoice() {
        ...
        for (Message m : msgs) {
            ...
            HybridChoice current = selectByAcoPso(m, candidates, congestion);
    ...
    private HybridChoice selectByAcoPso(Message m, List<Candidate> candidates, double congestion) {
        ...
        int popSize = Math.min(this.swarmSize, candidates.size());
        List<Candidate> population = new ArrayList<Candidate>(candidates.subList(0, popSize));

        // Missing GA: Map ACO baseScore directly into the GA placeholder so PSO functions normally
        ...
        Candidate best = null;
        for (Candidate c : population) {
            // Missing GA: Final score blends ACO directly with PSO
            c.finalScore = clamp((1.0 - this.omega2) * c.baseScore + this.omega2 * c.psoScore);
    ...
    private void applyCandidatePso(Message m, List<Candidate> population) {
        ...
        // Uses the ACO baseScore mapped to gaScore
        for (Candidate c : population) {
    ...
    private void syncDelivered(AcoPsoRouter other) {
        if (other == null) {
            return;
        }
